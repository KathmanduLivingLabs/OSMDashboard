	...
	if(bbox === '' && fromYear === '2010' && toYear === '2015') {
		switch(query_type) {
			case 'roads':
				...
				loadingDataDeffered[0].resolve(nepalStats.all[0], query_type);
				break;
			case 'waterways':
				//return nepalStats.all[1];
				loadingDataDeffered[1].resolve(nepalStats.all[1], query_type);
				break;
			case 'edu_institute':
				//return nepalStats.all[2];
				loadingDataDeffered[2].resolve(nepalStats.all[2], query_type);
				break;
			case 'buildings':
				//return nepalStats.all[3];
				loadingDataDeffered[3].resolve(nepalStats.all[3], query_type);
				break;
			case 'medical':
				//return nepalStats.all[4];
				loadingDataDeffered[4].resolve(nepalStats.all[4], query_type);
				break;
			case 'financial_institute':
				//return nepalStats.all[5];
				loadingDataDeffered[5].resolve(nepalStats.all[5], query_type);
				break;
			case 'gov_offices':
				//return nepalStats.all[6];
				loadingDataDeffered[6].resolve(nepalStats.all[6], query_type);
				break;
			case 'historic_sites':
				//return nepalStats.all[7];
				loadingDataDeffered[7].resolve(nepalStats.all[7], query_type);
				break;
			case 'natural_heritage':
				//return nepalStats.all[8];
				loadingDataDeffered[8].resolve(nepalStats.all[8], query_type);
				break;
			case 'tourist_interest':
				//return nepalStats.all[9];
				loadingDataDeffered[9].resolve(nepalStats.all[9], query_type);
				break;
			case 'settlement':
				//return nepalStats.all[10];
				loadingDataDeffered[10].resolve(nepalStats.all[10], query_type);
				break;
			case 'e_i_y':
				//return nepalStats.all[11];
				loadingDataDeffered[11].resolve(nepalStats.all[11], query_type);
				break;
			case 'users':
				//return nepalStats.all[12];
				loadingDataDeffered[12].resolve(nepalStats.all[12], query_type);
				break;
		}
	} else {
